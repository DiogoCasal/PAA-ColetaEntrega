...
def gerar_solucoes_possiveis(lojas, capacidade_caminhao):
    ...
    def backtrack(caminho, carga, distancia_total,custo_total):
        if len(caminho) == len(lojas):
            solucoes.append((caminho.copy(), distancia_total,custo_total))
            return
        ...
        for proxima_loja in lojas:
            if proxima_loja.numeroLoja not in caminho and proxima_loja.numeroLoja != atual:
                ...
                if nova_carga <= capacidade_caminhao:
                    nova_distancia += Loja.calcular_distancia(lojas[atual].x, lojas[atual].y, proxima_loja.x, proxima_loja.y)
                    novo_custo = Loja.calcular_gasto_combustivel(nova_distancia, nova_carga)
                    backtrack(caminho + [proxima_loja], nova_carga, nova_distancia, novo_custo)

    backtrack([lojas[0]], 0, 0,0)
    return solucoes
...
def resolver_problema_forca_bruta(lojas, capacidade_caminhao):
    ...
    caminho, distancia_total,custo_total = melhor_solucao
